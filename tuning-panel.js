...
const TUNING_FALLBACKS = {
    ...
    // ─── 相机 ────────────────────────────────────
    cameraZoomDamp: 3.2,
    cameraZoomOutDamp: 9.8,
    cameraPosDamp: 3.6,
    cameraContainUrgency: 2.2,
    cameraContainSafeMargin: 0.08,
    cameraMouseInfluence: 0.28,
    cameraMouseLeadMax: 220,
    cameraMouseDeadZone: 96,
    cameraForwardInfluence: 0.14,
    cameraLeadRelaxation: 0.42,
    cameraMinZoom: 0.22,
    cameraEmergencyMinZoom: 0.03,
    cameraMaxZoom: 1.12,
    cameraSpanPadding: 140,
    cameraEdgePadding: 72,
...
const TUNING_DEFS = [
    ...
    { key: 'cameraContainUrgency', label: '包边兜底强度', desc: '节点逼近屏幕边缘时，镜头额外加速多少去兜住', min: 0, max: 5, step: 0.05 },
    { key: 'cameraContainSafeMargin', label: '懒迭代容忍比', desc: '允许短时轻微出界多少后，镜头才明显切入兜底模式', min: 0, max: 0.4, step: 0.01 },
    ...
    { key: 'cameraForwardInfluence', label: '前向构图偏置', desc: '镜头默认沿当前流向或朝向，轻微顾头不顾腚', min: 0, max: 0.5, step: 0.01 },
    { key: 'cameraLeadRelaxation', label: '前探保留度', desc: '兜底时仍保留多少前探感，越高越有目标感', min: 0, max: 1, step: 0.02 },
    { key: 'cameraMinZoom', label: '舒适缩放下限', desc: '正常构图时，镜头不愿轻易缩得比这更小', min: 0.05, max: 1, step: 0.01 },
    { key: 'cameraEmergencyMinZoom', label: '紧急兜底下限', desc: '节点太多时，为了装下全体允许缩到多小', min: 0.02, max: 0.5, step: 0.01 },
    { key: 'cameraMaxZoom', label: '单体贴敷放大', desc: '独狼或者极少时的自动拉大上限', min: 0.5, max: 3, step: 0.02 },
    { key: 'cameraSpanPadding', label: '主体世界留白', desc: '围绕结构本身额外留多少世界空间', min: 0, max: 500, step: 10 },
    { key: 'cameraEdgePadding', label: '屏幕边缘留白', desc: '屏幕四周预留像素边距，避免节点和连线顶满屏幕', min: 0, max: 200, step: 2 },
...
const COMPOSITE_TUNING_MAPS = {
    ...
    feelCameraDirector: [
        ...
        { key: 'cameraZoomOutDamp', min: 6.2, max: 13.0 },
        { key: 'cameraPosDamp', min: 2.4, max: 5.8 },
        { key: 'cameraContainUrgency', min: 1.0, max: 3.6 },
        { key: 'cameraContainSafeMargin', min: 0.03, max: 0.14 },
        { key: 'cameraMouseInfluence', min: 0.12, max: 0.54 },
        { key: 'cameraMouseLeadMax', min: 120, max: 280 },
        { key: 'cameraMouseDeadZone', min: 140, max: 40 },
        { key: 'cameraForwardInfluence', min: 0.06, max: 0.24 },
        { key: 'cameraLeadRelaxation', min: 0.22, max: 0.72 },
        { key: 'cameraMinZoom', min: 0.32, max: 0.16 },
        { key: 'cameraEmergencyMinZoom', min: 0.12, max: 0.03 },
        { key: 'cameraSpanPadding', min: 120, max: 180 },
        { key: 'cameraEdgePadding', min: 56, max: 92 }
    ]
};
