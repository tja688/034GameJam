...
const TUNING_FALLBACKS = {
    ...
    gameplayPreyCameraScaleEnabled: true,
    gameplayPreyCameraScaleExponent: 0.55,
    gameplayPreyCameraScaleCap: 8,
    ...
    gameplayBasicGlobalSizeMul: 1,
    gameplayBasicGlobalHealthMul: 1,
    gameplayBasicGlobalSpeedMul: 1,
    gameplayBasicGlobalYieldMul: 1,
    gameplayEliteGlobalSizeMul: 1,
    gameplayEliteGlobalHealthMul: 1,
    gameplayEliteGlobalSpeedMul: 1,
    gameplayEliteGlobalYieldMul: 1,
    gameplayObjectiveGlobalSizeMul: 1,
    gameplayObjectiveGlobalHealthMul: 1,
    gameplayObjectiveGlobalSpeedMul: 1,
    gameplayObjectiveGlobalYieldMul: 1,
    gameplayBasicSizeJitter: 0.18,
    gameplayEliteSizeJitter: 0.08,
    gameplayObjectiveSizeJitter: 0.03,
    gameplayPreyGlobalSizeMul: 1.8,
    gameplayPreySmallSizeMul: 1.62,
    gameplayPreyMediumSizeMul: 1.84,
    gameplayPreyLargeSizeMul: 2.16,
    gameplayPreySimpleBiteDps: 18,
    gameplayPreySimpleTTKSmall: 1.35,
    gameplayPreySimpleTTKLarge: 4.8,
    gameplayPreyVolumeRefSmallRadius: 18,
    gameplayPreyVolumeRefLargeRadius: 42,
    gameplayPreyVolumeProjectionExponent: 2,
    ...buildPreyRuleSizeFallbacks(),
    ...buildEncounterSpeciesFallbacks(),
    ...buildEncounterBehaviorFallbacks(),
    ...
    gameplayLootBurstSpeedMul: 1,
    gameplayLootBurstSpreadMul: 1,
    gameplayLootVisualSizeMul: 1,
    gameplayLootAbsorbSpeedMul: 1,
    ...
    gameplayPreyBehaviorLodEnabled: true,
    gameplayPreyBehaviorLodViewMul: 0.72,
    gameplayPreySimpleFleeMode: false,
    ...
    gameplayMetabolismNodeSoftStart: 18,
    gameplayMetabolismNodeOverflowMul: 0.12,
    ...
    gameplayPulseMetabolismExtraRunnerMul: 0.3,
    ...
    gameplayEliteSpinnerSpinDamageMul: 0.72,
    gameplayEliteSpinnerRestDamageMul: 1.12,
    gameplayEliteBruteBaseDamageMul: 0.82,
    gameplayEliteBruteEncircleDamageMul: 0.24,
    gameplayEliteBruteCutterDamageMul: 0.26,
    gameplayEliteDartBaseDamageMul: 0.68,
    gameplayEliteDartEncircleDamageMul: 0.54,
    gameplayEliteDartPressureDamageMul: 0.3,
    gameplayEliteDartFeedEncircleGate: 0.34,
    gameplayEliteDartFeedPenalty: 0.52,
...
const TUNING_DEFS = [
    ...
    { key: 'gameplayMaxEnergy', label: '基础能量池', desc: '不算节点扩容时的基础能量上限。', min: 20, max: 1200, step: 1 },
    { key: 'gameplayMaxEnergyPerNode', label: '每节点扩容', desc: '每多 1 个节点，最大能量额外加多少。大体型想保住续航，先拧这项。', min: 0, max: 12, step: 0.05 },
    ...
    { key: 'gameplayStageSpawnCapMul', label: '刷怪上限倍率', desc: '统一缩放各阶段 spawnCap。高节点实验时可以大胆往上拉。', min: 0.25, max: 8, step: 0.05 },
    ...
    { key: 'gameplayPreyEncounterDensityMul', label: '猎物分布密度', desc: '扫图时综合遇怪频率。越高越容易持续遇到可猎杀目标；越低越稀疏。', min: 0.35, max: 8, step: 0.05 },
    { key: 'gameplayPreyCameraScaleEnabled', label: '镜头扩容联动', desc: '自动镜头拉远后，按真实可见世界面积同步放大刷怪预算、补怪范围和可见填充。', type: 'toggle' },
    { key: 'gameplayPreyCameraScaleExponent', label: '镜头扩容增幅', desc: '镜头越远，猎物预算放大得有多狠。100 节点压不满就继续往上拧。', min: 0.2, max: 1.2, step: 0.02 },
    { key: 'gameplayPreyCameraScaleCap', label: '镜头扩容上限', desc: '限制自动镜头最远时最多把预算放大几倍，避免直接冲爆模拟。', min: 1, max: 24, step: 0.25 },
    { key: 'gameplayPreyInitialDensityMul', label: '初始铺场密度', desc: '按每条 spawn rule 的 desired 扩大开局猎物数量。', min: 0.25, max: 12, step: 0.05 },
    { key: 'gameplayPreyInitialCountBonus', label: '初始数量补正', desc: '每条 spawn rule 额外再加几个。', min: -4, max: 64, step: 1 },
    ...
    { key: 'gameplayPreySimpleTTKSmall', label: '小体型击杀时长', desc: '当前相机视角下，参考小体型 prey 被单路啃完的目标秒数。', min: 0.2, max: 12, step: 0.05 },
    { key: 'gameplayPreySimpleTTKLarge', label: '大体型击杀时长', desc: '当前相机视角下，参考大体型 prey 被单路啃完的目标秒数。', min: 0.2, max: 24, step: 0.05 },
    { key: 'gameplayPreySimpleBiteDps', label: '单路啃食 DPS', desc: '每一条附着咬链的基础每秒伤害，用于把“击杀时长”换算成生命值。', min: 1, max: 120, step: 0.5 },
    { key: 'gameplayPreyVolumeRefSmallRadius', label: '小体积参考半径', desc: '体积血量曲线里“小对象”的参考世界半径。', min: 4, max: 120, step: 1 },
    { key: 'gameplayPreyVolumeRefLargeRadius', label: '大体积参考半径', desc: '体积血量曲线里“大对象”的参考世界半径。', min: 8, max: 200, step: 1 },
    { key: 'gameplayPreyVolumeProjectionExponent', label: '体积映射指数', desc: '把屏幕相对尺寸映射到“相对体积”的指数。2 更接近面积，3 更接近体积。', min: 1, max: 3.5, step: 0.05 },
    ...
    { key: 'gameplayLootBurstSpeedMul', label: '收获炸开速度', desc: '死亡后可收取碎片四射飞溅的整体速度。', min: 0.25, max: 3.5, step: 0.05 },
    { key: 'gameplayLootBurstSpreadMul', label: '收获炸开范围', desc: '死亡后碎片炸开的角度分布有多散。', min: 0.25, max: 2.5, step: 0.05 },
    { key: 'gameplayLootVisualSizeMul', label: '收获碎片大小', desc: '可收取资源碎片的视觉尺寸倍率。', min: 0.4, max: 3.5, step: 0.05 },
    { key: 'gameplayLootAbsorbSpeedMul', label: '收获吸收速度', desc: '圆形节点回收碎片的整体效率。', min: 0.25, max: 4, step: 0.05 },
    ...
    ...buildEncounterClassTuningDefs(),
    ...buildStageCarryTuningDefs(),
    ...buildEncounterSpeciesTuningDefs(),
    ...buildEncounterBehaviorTuningDefs(),
    ...
    { key: 'graphicsRenderPreySignalsEnabled', label: '猎物信号圈', desc: '精英脉冲、目标双环和其他猎物警示圈。', type: 'toggle' },
    ...
    { key: 'graphicsRenderPreyGuardRingsVisible', label: '守卫脉冲环', desc: '精英进入高威胁阶段时触发的 guard pulse ring。', type: 'toggle' },
    ...
    { key: 'gameplayMetabolismNodeWeight', label: '节点加价权重', desc: '节点越多，每次脉动额外越贵。这里只管基础斜率。', min: 0, max: 2.5, step: 0.01 },
    { key: 'gameplayMetabolismNodeSoftStart', label: '节点软拐点', desc: '超过多少个额外节点后，维护费不再继续按原斜率线性暴涨。', min: 0, max: 240, step: 1 },
    { key: 'gameplayMetabolismNodeOverflowMul', label: '超量节点折算', desc: '超过软拐点后的节点，按多少比例继续计入代谢。1 = 不压缩，越低越宽容大集群。', min: 0, max: 1.5, step: 0.01 },
    ...
    { key: 'gameplayPulseMetabolismBase', label: '单次脉动基础耗能', desc: '每次脉动至少消耗多少。', min: 0.01, max: 3, step: 0.01 },
    { key: 'gameplayPulseMetabolismExtraRunnerMul', label: '额外脉冲折算', desc: '前两个脉冲球以后，额外脉冲有多少比例继续计入代谢。越低越不会因为多脉冲瞬间雪崩。', min: 0, max: 1.5, step: 0.01 },
    ...
    { key: 'gameplayPreySimpleFleeMode', label: '本能逃离模式', desc: '关闭复杂状态机，让所有猎物只保留朝远离方向逃离的本能，用来测试大集群爽杀和性能上限。', type: 'toggle' },
    ...
    { section: '精英 / Objective 受击乘区', sectionDesc: '新精英与 objective 的专属受击公式。' },
    { key: 'gameplayEliteSpinnerSpinDamageMul', label: '旋刃方·旋转受击', desc: '旋刃方处于危险旋转期时，你对它的伤害倍率。', min: 0.1, max: 2, step: 0.01 },
    { key: 'gameplayEliteSpinnerRestDamageMul', label: '旋刃方·疲劳受击', desc: '旋刃方疲劳空窗时，你对它的伤害倍率。', min: 0.1, max: 2.5, step: 0.01 },
    { key: 'gameplayEliteBruteBaseDamageMul', label: '巨三角·基础受击', desc: '巨三角的基础伤害倍率。', min: 0.1, max: 2, step: 0.01 },
    { key: 'gameplayEliteBruteEncircleDamageMul', label: '巨三角·包围加伤', desc: 'encircle 对巨三角的额外伤害贡献。', min: 0, max: 1.5, step: 0.01 },
    { key: 'gameplayEliteBruteCutterDamageMul', label: '巨三角·刀口加伤', desc: 'cutterCount 对巨三角的额外伤害贡献。', min: 0, max: 1.5, step: 0.01 },
    { key: 'gameplayEliteDartBaseDamageMul', label: '飞梭·基础受击', desc: '飞梭的基础伤害倍率。', min: 0.1, max: 2, step: 0.01 },
    { key: 'gameplayEliteDartEncircleDamageMul', label: '飞梭·包围加伤', desc: 'encircle 对飞梭的额外伤害贡献。', min: 0, max: 1.5, step: 0.01 },
    { key: 'gameplayEliteDartPressureDamageMul', label: '飞梭·压力加伤', desc: 'pressure 对飞梭的额外伤害贡献。', min: 0, max: 1.5, step: 0.01 },
    { key: 'gameplayEliteDartFeedEncircleGate', label: '飞梭·吞食包围门槛', desc: 'feed 模式低于这个包围度会被罚。', min: 0, max: 1, step: 0.01 },
    { key: 'gameplayEliteDartFeedPenalty', label: '飞梭·吞食惩罚', desc: '围得不够紧时，feed 对飞梭的伤害倍率。', min: 0, max: 1, step: 0.01 },
