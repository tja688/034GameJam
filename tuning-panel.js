...
const TUNING_FALLBACKS = {
    ...
    clusterVolumeCameraPaddingBoost: 160,
    clusterVolumeBurstAssist: 0.28,
    clusterVolumeCenterContract: 0.12,
    clusterVolumePulseReach: 0.18,
    clusterVolumeSideReach: 0.34,
    burstCenterRadiusFactor: 0.26,
    burstCenterRadiusMin: 76,
    burstChaseRadiusFactor: 0.72,
    burstChaseRadiusMin: 170,
    burstBreakRadiusFactor: 1.18,
    burstBreakRadiusMin: 280,
    ...
    // ─── 相机 ────────────────────────────────────
    cameraZoomDamp: 1.8,
    cameraZoomOutDamp: 4.4,
    cameraZoomTrackDamp: 2.6,
    cameraPosDamp: 4.2,
    cameraLeadDamp: 10.5,
    cameraFocusTrackDamp: 7.2,
    cameraContainUrgency: 2.2,
    cameraContainSafeMargin: 0.08,
    cameraMouseInfluence: 0.46,
    cameraMouseLeadMax: 260,
    cameraMouseDeadZone: 110,
    cameraForwardInfluence: 0.16,
    cameraLeadRelaxation: 0.42,
    cameraClusterTether: 0.12,
    cameraMinZoom: 0.03,
    cameraEmergencyMinZoom: 0.03,
    cameraMaxZoom: 1.12,
...
const TUNING_DEFS = [
    ...
    { key: 'feelCameraBreathing', label: '视野呼吸感', desc: '控制集群尺度变化时的懒缩放、留白和全局感', min: 0, max: 1, step: 0.01 },
    ...
    { key: 'burstChaseRadiusMin', label: '追击圈层最小半径', desc: '小体型时仍能触发追击段', min: 40, max: 360, step: 2 },
    { key: 'burstBreakRadiusFactor', label: '突破圈层系数', desc: '远距离甩鼠标后更容易在此圈外释放', min: 0.4, max: 2.4, step: 0.02 },
    { key: 'burstBreakRadiusMin', label: '突破圈层最小半径', desc: '保障“远甩一下就要冲”的触发区间', min: 80, max: 520, step: 4 },
    ...
    { key: 'clusterVolumeCameraPaddingBoost', label: '体积镜头留白', desc: '舒张时镜头自动额外拉开多少', min: 0, max: 400, step: 5 },
    { key: 'clusterVolumeBurstAssist', label: '爆发自动舒张', desc: '追击/突破时给体积自动加多少临时外扩', min: 0, max: 1, step: 0.02 },
    { key: 'clusterVolumeCenterContract', label: '中心收束回压', desc: '鼠标回到质心附近时自动轻微收缩', min: 0, max: 0.5, step: 0.01 },
    ...
    { key: 'cameraZoomDamp', label: '收镜阻尼', desc: '集群缩小时，镜头回到更近构图的速度', min: 0.4, max: 6, step: 0.05 },
    { key: 'cameraZoomOutDamp', label: '拉远阻尼', desc: '集群变大时，镜头往外退的速度', min: 0.4, max: 10, step: 0.05 },
    { key: 'cameraZoomTrackDamp', label: '缩放懒跟随', desc: '镜头对集群体积变化的基础反应速度，稳定时越低越稳', min: 0.3, max: 8, step: 0.05 },
    { key: 'cameraPosDamp', label: '镜头主体跟随', desc: '镜头本体追到目标焦点的速度', min: 1, max: 8, step: 0.05 },
    { key: 'cameraLeadDamp', label: '鼠标偏移净化', desc: '先把鼠标意图净化成干净偏移，再交给镜头去跟', min: 2, max: 18, step: 0.1 },
    { key: 'cameraFocusTrackDamp', label: '焦点净化', desc: '焦点目标二次滤波，越高越利落，越低越漂', min: 2, max: 14, step: 0.1 },
    { key: 'cameraMouseInfluence', label: '鼠标偏移权重', desc: '镜头偏移有多听鼠标的', min: 0, max: 1, step: 0.02 },
    { key: 'cameraMouseLeadMax', label: '鼠标偏移上限', desc: '鼠标把镜头最多能拉开多少世界距离', min: 40, max: 420, step: 5 },
    { key: 'cameraMouseDeadZone', label: '鼠标静区半径', desc: '屏幕中心附近多大范围内镜头不急着偏', min: 0, max: 260, step: 2 },
    { key: 'cameraForwardInfluence', label: '前向趋势偏置', desc: '额外沿流向/朝向给镜头一点趋势感', min: 0, max: 0.5, step: 0.01 },
    { key: 'cameraClusterTether', label: '集群牵引', desc: '集群很大时，镜头被体积中心轻微拉回多少', min: 0, max: 0.4, step: 0.01 },
    { key: 'cameraMinZoom', label: '视野下限', desc: '镜头最多允许缩到多小', min: 0.02, max: 0.4, step: 0.01 },
    { key: 'cameraMaxZoom', label: '贴近上限', desc: '结构很小时，镜头最多能贴多近', min: 0.5, max: 3, step: 0.02 },
    { key: 'cameraSpanPadding', label: '主体留白', desc: '围绕集群本体额外留多少世界空间', min: 0, max: 300, step: 5 },
...
const COMPOSITE_TUNING_MAPS = {
    ...
    feelCameraDirector: [
        { key: 'cameraMouseInfluence', min: 0.22, max: 0.58 },
        { key: 'cameraMouseLeadMax', min: 160, max: 340 },
        { key: 'cameraMouseDeadZone', min: 136, max: 58 },
        { key: 'cameraForwardInfluence', min: 0.08, max: 0.22 },
        { key: 'cameraClusterTether', min: 0.18, max: 0.06 },
        { key: 'cameraFocusTrackDamp', min: 5.2, max: 8.6 },
        { key: 'cameraPosDamp', min: 3.2, max: 5.0 }
    ],
    feelCameraGlide: [
        { key: 'cameraLeadDamp', min: 7.2, max: 12.5 },
        { key: 'cameraFocusTrackDamp', min: 5.6, max: 8.4 },
        { key: 'cameraPosDamp', min: 3.0, max: 4.8 },
        { key: 'cameraZoomTrackDamp', min: 3.2, max: 1.9 },
        { key: 'cameraZoomDamp', min: 2.4, max: 1.4 },
        { key: 'cameraZoomOutDamp', min: 5.2, max: 3.4 }
    ],
    feelCameraBreathing: [
        { key: 'cameraSpanPadding', min: 70, max: 160 },
        { key: 'cameraEdgePadding', min: 42, max: 92 },
        { key: 'cameraMinZoom', min: 0.06, max: 0.03 },
