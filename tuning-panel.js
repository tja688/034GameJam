...
const TUNING_DEFS = [
    ...
    { key: 'cameraLeadDamp', label: '鼠标偏移净化', desc: '先把鼠标意图净化成干净偏移，再交给镜头去跟。想要更敏捷就往高调。', min: 2, max: 28, step: 0.1 },
    { key: 'cameraFocusTrackDamp', label: '焦点净化', desc: '焦点目标二次滤波，越高越利落，越低越漂', min: 2, max: 18, step: 0.1 },
    { key: 'cameraMouseInfluence', label: '鼠标偏移权重', desc: '镜头偏移有多听鼠标的。现在允许调到非常明显。', min: 0, max: 1.8, step: 0.02 },
    { key: 'cameraMouseLeadMax', label: '鼠标偏移上限', desc: '鼠标把镜头最多能拉开多少世界距离', min: 40, max: 900, step: 10 },
    { key: 'cameraMouseDeadZone', label: '鼠标静区半径', desc: '屏幕中心附近多大范围内镜头不急着偏。想要更灵敏就往小调。', min: 0, max: 320, step: 2 },
    { key: 'cameraForwardInfluence', label: '前向趋势偏置', desc: '额外沿流向/朝向给镜头一点趋势感', min: 0, max: 1.2, step: 0.01 },
...
const COMPOSITE_TUNING_MAPS = {
    ...
    feelCameraDirector: [
        { key: 'cameraMouseInfluence', min: 0.22, max: 0.95 },
        { key: 'cameraMouseLeadMax', min: 160, max: 520 },
        { key: 'cameraMouseDeadZone', min: 136, max: 24 },
        { key: 'cameraForwardInfluence', min: 0.08, max: 0.44 },
        { key: 'cameraClusterTether', min: 0.18, max: 0.06 },
        { key: 'cameraFocusTrackDamp', min: 5.2, max: 10.8 },
        { key: 'cameraPosDamp', min: 3.2, max: 5.6 }
    ],
    feelCameraGlide: [
        { key: 'cameraLeadDamp', min: 7.2, max: 16.0 },
        { key: 'cameraFocusTrackDamp', min: 5.6, max: 10.5 },
